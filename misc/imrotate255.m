...
%   Class Support
%   -------------
%   The input image can be numeric, logical or categorical.  The output
...
if (isempty(A))
    ...
    if rem(ang,90) == 0
        ...
        if useIPP(A)
            ...
            if(any(twod_size==1))
                A = padarray(A,[2 2],255);
            end
                        
            if isreal(A)
                B = images.internal.builtins.imrotate(A,ang,outputSize,method);
            else
                % If input is complex valued, call imrotatemex on real and
                % imaginary parts separately then combine results.
                B = complex( images.internal.builtins.imrotate(real(A),ang,outputSize,method),...
                    images.internal.builtins.imrotate(imag(A),ang,outputSize,method));
            end          
...
function [A,ang,method,bbox,catConverter,isInputCategorical] = parse_inputs(varargin)
...
% validate image
A = varargin{1};
validateattributes(A,{'numeric','logical','categorical'},{},mfilename,'input image',1);
...
if nargin==3
    ...
    if ~isempty(catConverter) && ~strcmpi(method,'nearest')
...
elseif nargin==4
    ...
    if isInputCategorical && ~strcmpi(method,'nearest')
        error(message('MATLAB:images:validate:badMethodForCategorical'))
    end
end
end
