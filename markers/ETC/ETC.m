...
if (numel(varargin) == 0)
    ...
    if  isscalar(varargin{1}) && isreal(varargin{1}) && round(varargin{1}) == varargin{1} && isnumeric(varargin{1}) && varargin{1} >= 0 
       NumBins = varargin{1};
    else
        error('NumBins should an integer greater than or equal to zero');
    end
end
...
%----------------------------------------------------
function [SymSeq] = Partition(InputTS,NumBins)
