function netTransferEntropy = netTE(combinedTE, focusAgent)
    ...
    % init netTE matrix 
    netTransferEntropy = zeros(length(timePeriods), numAgents+1);
    
    % init time steps 
    netTransferEntropy(:,numAgents+1) = timePeriods;
    ...
    for timeIter = timePeriods
        ...
        for externalAgentIndex = 1:length(externalAgent)
            ...
            % calculate netTE as netTE = TE(s-t) - TE(t-s) for all s,t pairs, s != t
            netTransferEntropy(timeIter,externalAgent(externalAgentIndex)) = nansum([meanOutgoingTE, -meanIncomingTE]);
        end
    end  
end